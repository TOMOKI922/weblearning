...
if(!empty($_POST)){
  ...
  $addr = (isset($_POST['addr'])) ? $_POST['addr'] : '例：英語区英語氏２０－２';
  $age = $_POST['age'];
  $email = $_POST['email'];
  ...
  if(empty($err_msg)){
    ...
    try{
      $dbh = dbConnect();
      $sql = 'UPDATE users SET username = :name, tel = :tel, zip = :zip, addr = :addr, age = :age, email = :email, pic = :pic WHERE id = :u_id';
      $data = array(':name' => $username, ':tel' => $tel, ':zip' => $zip, ':addr' => $addr, ':age' => $age, ':email' => $email, ':pic' => $pic, ':u_id' => $dbFormData['id']);
...
<body class="page-profEdit page-2colum page-logined">
  ...
  <div id="contents" class="site-width">
    ...
    <section id="main" class="profEdit">
      <div class="form-container profEdit">
        <form action="" method="post" class="form" enctype="multipart/form-data">
          ...
          <label class="<?php if(!empty($err_msg['addr'])) echo 'err'; ?>">
            住所
          <input type="text" name="addr" value="<?php if(!empty(getFormData('addr'))){echo getFormData('addr');}?>">
          </label>
        <div class="area-msg">
        ...
        <label class="area-drop <?php if(!empty($err_msg['pic'])) echo 'err'; ?>" style="height:370px;line-height:370px;">
          ...
          <img src="<?php getFormData('pic'); ?>" alt="" class="prev-img" style="<?php if(empty(getFormData('pic'))) echo 'display:none;' ?>">
            ドラッグ＆ドロップ
          </label>
          <div class="area-msg">
